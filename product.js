...
function openProductModal(card) {
    ...
    currentProduct = {
        ...
        images: safeParseJSON(card.dataset.images, [card.dataset.images]),
    ...
    if (thumbRow) {
        ...
        if (thumbRow.firstChild) {
            thumbRow.firstChild.classList.add('active');
        }
    }
...
function addToCartFromModal() {
    ...
    const cartItem = {
        ...
        total: (currentProduct.price * yards).toFixed(2)
    };
                ...
                li.innerHTML = `
                    <div class="cart-item-left">
                        ...
                        <div class="cart-item-details">
                            ${item.yards} yd • ${item.color} • $${(item.price * item.yards).toFixed(2)}
                        </div>
                    </div>
...
function checkout() {
    ...
    localStorage.setItem('fabricFactoryCheckout', JSON.stringify(checkoutCart));
    
    const total = checkoutCart.reduce((sum, item) => sum + parseFloat(item.total), 0);
